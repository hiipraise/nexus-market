...
export async function GET(req: NextRequest) {
  try {
    ...
    // Revenue by day
    const revenueByDay = recentOrders.reduce((acc: Record<string, number>, order) => {
    ...
    const productRevenue = recentOrders.reduce(
      (acc: Record<string, { name: string; revenue: number; units: number }>, order) => {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        period: {
          range,
          revenue:      recentOrders.reduce((s, o) => s + o.items.filter(i => String(i.vendorId) === String(vendor._id)).reduce((a, i) => a + i.vendorAmount, 0), 0),
