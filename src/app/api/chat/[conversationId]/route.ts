...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    await ChatMessage.updateMany(
      { conversationId: params.conversationId, senderId: { $ne: session!.user.id }, isRead: false },
      { isRead: true }
    )
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    if (!content?.trim()) return NextResponse.json({ success: false, error: 'Message required' }, { status: 400 })
