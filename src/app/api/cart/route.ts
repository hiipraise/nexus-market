...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session) {
      return NextResponse.json(
        { success: false, error: "Login required to add to cart" },
        { status: 401 },
      );
    }
    ...
    if (!variant) {
      return NextResponse.json(
        { success: false, error: "Size not available" },
        { status: 400 },
      );
    }
    ...
    const existingIdx = cart.items.findIndex(
      (i: any) => String(i.productId) === productId && i.size === size,
    );
    if (existingIdx >= 0) {
  ...
  try {
    ...
    if (!target) {
      return NextResponse.json(
        {
          success: false,
          error: "User not found or their username is private",
        },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: "Failed to share cart" },
      { status: 500 },
    );
  }
}
